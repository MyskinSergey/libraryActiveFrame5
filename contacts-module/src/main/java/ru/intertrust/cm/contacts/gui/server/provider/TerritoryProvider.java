 ...
 */
@Component
@ComponentName("territory.provider")
public class TerritoryProvider implements EnumerationMapProvider {
    @Override
    @SuppressWarnings("rawtypes")
    public Map<String, Value> getMap(WidgetContext widgetContext) {
        Map<String,Value> values = new HashMap<>();
        values.put("Город",new LongValue(0));
        values.put("Регион",new LongValue(1));
        return values;
    }
}
