...
@ComponentName("activate.contact.name.visibility.checker")
public class ActivateContactNameVisibilityChecker implements ActionVisibilityChecker {
    ...
    public boolean isVisible(ActionVisibilityContext context) {
        ...
        if (currentDomainObject != null && currentDomainObject.getId() != null) {
            if(currentDomainObject.getStatus() != null) {
                if (!dobjectsManager.getStatusById(currentDomainObject.getStatus()).equals(STATUS_ACTIVATED)) {
                    return true;
                }
            }
        }
