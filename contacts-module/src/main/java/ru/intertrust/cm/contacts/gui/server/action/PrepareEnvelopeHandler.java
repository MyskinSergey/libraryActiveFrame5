 ...
 */
@ComponentName("prepare.envelope.action")
public class PrepareEnvelopeHandler extends ActionHandler<ActionContext, SimpleActionData> {
    ...
    public SimpleActionData executeAction(ActionContext context) {
        ...
        if (rootObjectId != null) {
            ...
            if (!dobjectsManager.getStatusById(rootDomainObject.getStatus()).equals(STATUS_READY)) {
                dobjectsManager.changeStatusForDo(rootDomainObject.getId(), STATUS_READY);
            }
        }
        return actionData;
    }
